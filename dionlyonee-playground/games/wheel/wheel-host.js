...
function init() {
  ...
  RoomSync.onStateChange((incoming) => {
    // Only take over local state if this isn't just our own echo
    state = incoming;
    render();
  });
...
async function pickPuzzle() {
  ...
  if (!puzzle) {
    alert('Every puzzle in this category has been used this session. Pick a different category, or start a new room to reset.');
  }
  return puzzle;
}
